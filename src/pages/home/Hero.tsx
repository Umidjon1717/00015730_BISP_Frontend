      ...
      <div className="flex items-center container justify-end max-lg:justify-center h-full relative z-10">
        <div className="text-start bg-white dark:bg-zinc-900 dark:bg-opacity-75 bg-opacity-70 p-8 max-sm:p-4 rounded-lg max-w-lg lg:max-w-md md:max-w-sm sm:max-w-xs max-sm:max-w-[90%]">
          ...
          <p className="text-gray-400 mb-6 text-xl lg:text-lg md:text-base sm:text-sm max-sm:text-xs max-sm:mb-3">
            Elevate your space with thoughtfully crafted furniture—modern
            comfort, timeless design, and quality you can feel every day.
          </p>
          <button onClick={() => navigate("/shop")} className="w-[210px] h-[60px] max-sm:text-[9px] lg:w-[180px] lg:h-[50px] md:w-[160px] md:h-[45px] sm:w-[140px] sm:h-[40px] max-sm:w-[100px] max-sm:h-[25px] bg-bg-primary text-white px-6 py-3 max-sm:px-2 max-sm:py-1 font-bold text-base max-sm:text-xs rounded-sm hover:bg-yellow-700 duration-300 max-sm:font-medium">
            BUY NOW
          </button>
        </div>
      </div>
